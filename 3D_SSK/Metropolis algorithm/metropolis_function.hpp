...
void initialize_main(char *len, char *seq_filename, char *conf, char *temp_arg, char *iter_arg) {
	...
	iter_count = atoi(iter_arg);
	T = atof(temp_arg);

	char temp[100];
	FILE *fp_minenergy = fopen("../Energy matrix, statistics and other calculations/Output_sequences_statistics/min_energy.txt", "r");
...
void file_open(int simul_no) {
	stringstream ss;
	ss << "Output/" << simul_no << "_iterations.txt";
	outfile1.open(ss.str().c_str());
	outfile1 << "Iteration_Number  Configuration  Energy  Contact_count\n";

	stringstream ss1;
	ss1 << "Output/" << simul_no << "_min_energy_config_stats.txt";
	outfile2.open(ss1.str().c_str());
	outfile2 << "Iteration_Number  Configuration  Energy  Contact_count\n";

	stringstream ss2;
	ss2 << "Output/" << simul_no << "_contact_frequency_stats.txt";
	outfile3.open(ss2.str().c_str());
	outfile3 << "Contact_count Configuration_count\n";

	stringstream ss3;
	ss3 << "Output/" << simul_no << "_min_energy_till_this_block_stats.txt";
	outfile4.open(ss3.str().c_str());
